(ns intro-to-ec.heuristic-search-vertical
...
(defn search
  [{:keys [get-next-node add-children]}
   {:keys [goal? make-children heuristic-ver]}
   start-node
   max-calls
   ]
  (let [goal [0 0]]
  (loop [frontier (pm/priority-map start-node (heuristic-ver start-node goal)),
    ...
    (let [current-node (get-next-node frontier)]
      (cond
        ...
        (let [kids (remove-previous-states
          ...
          (recur
           (add-children (pop frontier) (map (fn [kid] [kid (heuristic-ver kid goal)]) kids )),
